...
describe('POST /auth/register', () => {
  const validBody = { email: 'test@gmail.com', password: PASSWORD, name: 'Test User' };

  it('creates a user and returns 201 with a token and safe user fields', async () => {
    findUnique.mockResolvedValue(null);
    create.mockResolvedValue(makeUser());

    const res = await server.inject({ method: 'POST', url: '/auth/register', payload: validBody });
  ...
  it('bcrypt-hashes the password before persisting (never plaintext)', async () => {
    ...
    await server.inject({ method: 'POST', url: '/auth/register', payload: validBody });
  ...
  it('returns 400 when the password is shorter than 8 characters', async () => {
    const res = await server.inject({
      method: 'POST',
      url: '/auth/register',
      payload: { email: 'test@gmail.com', password: '1234567', name: 'Test User' },
    });
...
describe('POST /auth/login', () => {
  ...
  it('returns 200 with a token for correct credentials', async () => {
    ...
    const res = await server.inject({ method: 'POST', url: '/auth/login', payload: validBody });
